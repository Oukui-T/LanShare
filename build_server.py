...
def build():
    ...
    # 清理旧的分发目录以防文件句柄冲突
    dist_server_dir = os.path.join("dist", "server")
    if os.path.exists(dist_server_dir):
        try:
            shutil.rmtree(dist_server_dir, ignore_errors=True)
        except Exception:
            pass
    ...
    cmd = [
        ...
        "--windows-icon-from-ico=icon/server.ico",
        "--windows-uac-admin",
        ...
        "--output-filename=LanShareServer.exe",
        "--assume-yes-for-downloads",
        "--show-progress",
        "server_app.py"
    ]
    print("==================================================")
    print(" 正在使用 Nuitka 编译服务端 (含管理员 UAC 提权)...")
    print("==================================================")
    print("执行命令:", " ".join(cmd))
    subprocess.run(cmd, check=True)
    
    dist_dir = os.path.join("dist", "server", "server_app.dist")
    gui_exe = os.path.join(dist_dir, "LanShareServer.exe")
    debug_exe = os.path.join(dist_dir, "LanShareServer_debug.exe")
    ...
    print("\n==================================================")
    print("[OK] 服务端封装完成！生成了两个版本的可执行文件：")
